...
export default function StudentRegistration() {
  ...
  const validateForm = () => {
    ...
    if (
      formData.contact1_phone &&
      !validatePhoneNumber(formData.contact1_phone)
    ) {
    ...
    if (hasSecondaryContact) {
      if (!formData.contact2_name) {
        newErrors.contact2_name =
          "Name is required if adding a secondary contact";
      }
      if (!formData.contact2_relationship) {
        newErrors.contact2_relationship =
          "Relationship is required if adding a secondary contact";
      }
      if (
        formData.contact2_phone &&
        !validatePhoneNumber(formData.contact2_phone)
      ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        ...
        contact2_phone: formData.contact2_phone
          ? formData.contact2_phone.replace(/\D/g, "")
          : null,
